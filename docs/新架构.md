...
>
> **Session UI 状态分类**：owned / foreign / takeover / orphan 的判定规则见 §4.3 和 §4.3.1。前端 sidebar 只展示 **owned**（主列表，可操作）和 **foreign**（"Foreign Sessions" 手风琴，只读）；**takeover** 和 **orphan** 不进 sidebar，由 `OrphanCleanModal`（扫帚按钮）通过独立的磁盘扫描路径发现和处理。
...
| **C4** | 无记录 | `.agentdock/worktrees/*` 目录存在 | 外部/旧版本建，或 purge 后磁盘瞬态残留 | **永不自动删**：UI 提示"领养或清理"（瞬态残留下一轮自消）。磁盘完整性判定见 §4.3.1：完整目录标 takeover，残缺目录标 orphan |
...
#### 4.3.1 客户端 orphan 判据（磁盘完整性判定）

客户端 `syncProject()` 扫到磁盘 worktree 但 daemon 不认时（v2Snapshot 无此路径），按**磁盘完整性**判定归类，**不依赖 daemon 状态**（daemon 可能已崩、WAL 可能丢失）：

| 磁盘状态 | 归类 | 处置 |
| --- | --- | --- |
| 目录存在 + 非空 + 有 `.git` | **takeover**（完整） | 标记为可接管；后续功能：用户可接管成为新 owner |
| 目录不存在 / 空 / 缺 `.git` | **orphan**（不完整） | 不进 sidebar 主列表；由 `OrphanCleanModal`（扫帚按钮）处理 |

原则：
- **判据只看磁盘，不看 daemon**——daemon 可能不存活，其状态不可靠。
- **takeover ≠ foreign**：foreign 是"别人活着，我只读"；takeover 是"没人了但东西还在，我可以接手"。
- takeover 和 orphan 都**不进 sidebar 主列表**——当前版本只做分类不进 sidebar，接管功能后续迭代。
- 磁盘完整性判定在客户端本地完成，不发 IPC、不查 daemon。
