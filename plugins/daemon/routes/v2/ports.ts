...
export function registerClaim(app: Hono, ctx: DaemonContext): void {
  app.post("/claim", zValidator("json", ClaimSchema, zodErrorHandler), async (c) => {
  ...
  app.post("/release", zValidator("json", ReleaseSchema, zodErrorHandler), async (c) => {
    const body = c.req.valid("json");
    try {
  ...
  app.post("/reassign", zValidator("json", ReassignSchema, zodErrorHandler), async (c) => {
    const body = c.req.valid("json");
    try {
      const result = await ctx.mutex.runExclusive("state", async () => {
        ctx.stateV2.assertFencingToken(body.sessionId, body.fencingToken);
