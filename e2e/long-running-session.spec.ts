 ...
 * verifies the terminal remains connected and the session remains stable
...
function writeProjectWithSlowHook(dir: string): void {
  ...
  writeFileSync(join(dir, "slow-hook.js"), "setTimeout(() => {}, 5000);", "utf-8");
  writeFileSync(
...
test.describe("long-running session", () => {
  test("create session -> observe 30+ seconds -> stable card and terminal -> delete clean", async ({
    window,
    dataDir,
    pageErrors,
    rendererLog,
    ...
    // the persisted session update to replace it with the real UUID before capturing it.
    // Otherwise we observe the tempId here and the real ID later, and
    // the stability check would fail on a false ID mismatch.
    // Best-effort: if the persisted update doesn't land within 20s (suite
    ...
    // ports may not be visible while session creation is still settling; the observation
    ...
    while (Date.now() - startTime < observeDuration) {
      ...
      expect(termStatus, `terminal disconnected during observation at check #${checkCount}`).toBe(
        "connected",
      );
