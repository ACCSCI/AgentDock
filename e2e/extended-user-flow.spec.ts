...
test("extended user flow: open project → create session → 3 terminals → close 1 → delete session", async ({ window }) => {
  ...
  await window.locator('[data-testid="new-session"]').waitFor({ state: "visible", timeout: 10_000 });
  ...
  const sessionId = await window.locator('[data-testid="session-card"]').last().getAttribute("data-session-id") ?? "";
  ...
  while (Date.now() < createDeadline) {
    const state = await window.evaluate((sid) => {
      const wrapper = document.querySelector(`[data-testid="session-card"][data-session-id="${sid}"]`);
  ...
  await window.locator('[data-testid="terminal-panel"]').waitFor({ state: "visible", timeout: 10_000 });
  ...
  expect(finalTabCount, "expected 3+ terminals after create").toBeGreaterThanOrEqual(initialTabs + 3);

  // Step 7: Close 1 terminal. Click the × button on the second tab.
  const firstCloseBtn = window.locator('[data-testid="terminal-tab"]').nth(1).locator('.terminal-tab-close');
  ...
  const sessionCard = window.locator(`[data-testid="session-card"][data-session-id="${sessionId}"]`);
  ...
  if (await closeSessionBtn.count() > 0) {
    ...
    await window.waitForTimeout(300);
    await window.locator('.context-menu-item').filter({ hasText: /删除|Delete/i }).first().click();
  }
