...
function createFakeWorktree(projectPath: string, sessionId: string, branch: string): void {
  ...
  writeFileSync(join(gitDir, "gitdir"), `${wtDir}/.git\n`, "utf-8");
}
...
test.describe("Global projects DB — worktree sync and session creation", () => {
  test("disk worktrees sync to sidebar and sessions:create works", async ({
    ...
    expect(existsSync(join(projectPath, ".agentdock", "worktrees", "aaa11111-aaa", ".git"))).toBe(
      true,
    );
    expect(existsSync(join(projectPath, ".agentdock", "worktrees", "bbb22222-bbb", ".git"))).toBe(
      true,
    );
    ...
    // because syncProject performs asynchronous project reconciliation, then
    // React Query re-fetches after invalidateQueries.
    const _syncLines = mainLog.filter((l) =>
      /syncProject|inserted disk|projects:create|projects:list|table:.*projects|seed migration|user_version/.test(
        l,
      ),
    );
    ...
    await expect
      .poll(
        async () => {
          ...
          const newSync = mainLog.filter((l) => /syncProject|query results/.test(l)).slice(-5);
  ...
  test("session creation error message does not contain 'no such table'", async ({
    ...
    const tableErrors = consoleErrors.filter(
      (e) => e.includes("no such table") && e.includes("projects"),
    );
