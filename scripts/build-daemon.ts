 ...
 * plugins/daemon.ts 预编译为 out/daemon/daemon.mjs。
...
async function main() {
  await build({
    ...
    bundle: true,
    outfile: resolve(root, "out/daemon/daemon.mjs"),
    platform: "node",
    target: "node24",
    format: "esm",
    external: [
  ...
  console.log("[build-daemon] daemon compiled to out/daemon/daemon.mjs");
}
